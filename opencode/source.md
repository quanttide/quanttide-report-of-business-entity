...
| `src/session/prompt.ts` | 2157 | 执行引擎：ReAct 循环、工具调用、子 Agent 调度 |
...
| 缺陷 | 位置 | 严重性 | 说明 |
|------|------|--------|------|
| 无重试机制 | L1821 | 🔴 高 | LLM 调用失败直接终止 |
| 无工具超时 | L578-608 | 🔴 高 | bash 等工具可无限阻塞循环 |
| 无死循环检测 | L1770-1832 | 🔴 高 | 重复调用同一工具不被阻止 |
| 全量读消息 | L1655 | 🟡 中 | 每轮从数据库读全部消息 |
| step 计数不持久化 | L1648 | 🟡 中 | 进程重启后 maxSteps 失效 |
| 同步阻塞压缩 | L1699-1707 | 🟡 中 | compaction 阻塞循环（前提：假设只有同步模式，需确认。若支持异步则不成立） |
| 硬编码退出条件 | L1672-1678 | 🟢 低 | `["tool-calls"]` 应使用常量 |
...
- ❌ 没有兜底退出（finish 永远 undefined 时只能靠 maxSteps）
- ❌ 没有检测死循环
...
以上来自代码分析推导，未收集实际用户反馈。

### 3.2 根因分析

| 原因 | 代码位置 | 机制 |
|------|----------|------|
| 无原始任务锚点 | L1657 | 只看最后一条用户消息 |
| Attention 衰减 | L1816 | 长对话中原始指令被推到中间低权重区域 |
| 压缩丢失细节 | L1655 | 压缩后关键约束被摘要替代 |
| 无计划跟踪 | 全局缺失 | 没有 TODO list |
| 无工作记忆 | 全局缺失 | 不维护已发现事实和决策 |

### 3.3 推荐改进方案

| 方案 | Token | 进度追踪 | 漂移检测 | 复杂度 | 排期 |
|------|-------|---------|---------|--------|------|
| 原始任务锚点 | 高 | ❌ | ❌ | 低 | P0 |
| 任务状态机 | 中 | ✅ | ⚠️ | 中 | P1 |
| 分层执行 | 低 | ✅ | ✅ | 高 | P2 |
...
- **P0（原始任务锚点）**：约 10 行改动，system prompt 加一条消息，不影响任何逻辑。零风险，可立即上线观察效果。
- **P1（任务状态机 + 自反思 + 约束持久化）**：三项均需新增数据结构和序列化逻辑。任务状态机依赖先确认 task 存储位置；约束持久化需要先定义约束提取规则；自反思需要先有漂移判定标准。
...
原因：
- 测试关注点在「机制能跑」，不是「Agent 跑得好」
- 每轮都要 mock LLM 回复，10 轮要写 10 个 `yield* llm.*`，编写成本高
- 没有定义「漂移」的可观测指标，无法断言
...
| Agent 测试 | `test/agent/agent.test.ts` | 732 |
| Prompt 测试 | `test/session/prompt.test.ts` | 1528+ |
